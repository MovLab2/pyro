...
const char * const BITCOIN_CONF_FILENAME = "pyro.conf";
const char * const BITCOIN_PID_FILENAME = "pyrod.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "pyro" is a composite category enabling all PYRO-related debug output
            if(ptrCategory->count(string("pyro"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\PYROCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\PYROCore
    // Mac: ~/Library/Application Support/PYROCore
    // Unix: ~/.pyrocore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "PYROCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/PYROCore";
#else
    // Unix
    return pathRet / ".pyrocore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty pyro.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override pyro.conf
