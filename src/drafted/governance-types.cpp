...
GovernanceObjectType GovernanceStringToType(std::string strType) {
    ...
    if(strType == "PYRONetwork") type = PYRONetwork;
    if(strType == "PYRONetworkVariable") type = PYRONetworkVariable;
...
std::string GovernanceTypeToString(GovernanceObjectType type) {
    ...
    if(type == PYRONetwork) s = "PYRONetwork";
    if(type == PYRONetworkVariable) s = "PYRONetworkVariable";
